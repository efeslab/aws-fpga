...
#pragma HLS INTERFACE m_axi depth=3*162*322 	port=image_in_raw_pad			offset=slave	bundle=gmem
#pragma HLS INTERFACE m_axi depth=405*16*16		port=conv_weight_1x1_all		offset=slave	bundle=gmem
#pragma HLS INTERFACE m_axi depth=22*16*3*3		port=conv_weight_3x3_all		offset=slave	bundle=gmem
#pragma HLS INTERFACE m_axi depth=67*16			port=bias_all					offset=slave	bundle=gmem

#pragma HLS INTERFACE m_axi depth=48*82*162		port=DDR_pool3_out_PL			offset=slave	bundle=gmem
#pragma HLS INTERFACE m_axi depth=96*42*82		port=DDR_pool6_out_PL			offset=slave	bundle=gmem

//#pragma HLS INTERFACE m_axi depth=96*82*162		port=DDR_pool_out_PL			offset=slave	bundle=INPUT

#pragma HLS INTERFACE m_axi depth=36*16*22*42	port=DDR_buf					offset=slave	bundle=gmem

#pragma HLS INTERFACE m_axi depth=5				port=predict_box				offset=slave	bundle=gmem
