...
{
    char *param_path = getenv("PARAM_PATH");
    //std::ifstream ifs_param("params_384_320_160_v2.bin", std::ios::in | std::ios::binary);
    std::ifstream ifs_param(param_path, std::ios::in | std::ios::binary);
    ...
    const uint64_t fix_image_raw_pad_addr = 0;
    const uint64_t fix_conv_weight_1x1_all_addr = MEM_1G;
    const uint64_t fix_conv_weight_3x3_all_addr = MEM_1G * 2;
    const uint64_t fix_bias_all_addr = MEM_1G * 3;
    const uint64_t DDR_pool_3_out_PL_addr = MEM_1G * 4;
    const uint64_t DDR_pool_6_out_PL_addr = MEM_1G * 5;
    const uint64_t DDR_buf_addr = MEM_1G * 6;
    const uint64_t predict_box_addr = MEM_1G * 7;

    const uint64_t fix_image_raw_pad_size = 3*162*322*sizeof(uint8);
    const uint64_t fix_conv_weight_1x1_all_size = 405*16*16*sizeof(FIX_16_1);
    const uint64_t fix_conv_weight_3x3_all_size = 22*16*3*3*sizeof(FIX_16_1);
    const uint64_t fix_bias_all_size = 67*16*sizeof(FIX_16_1);
    const uint64_t DDR_pool3_out_PL_size = 48*82*162*sizeof(FIX_FM);
    const uint64_t DDR_pool6_out_PL_size = 96*42*82*sizeof(FIX_FM);
...
extern "C" int hls_main()
{

	test_one_frame(getenv("FIG_PATH"));
