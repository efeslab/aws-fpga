...
void run_FPGA(int N_loc, int nt, float EPS, float *m, const particle_t *in_particles, particle_t *out_particles,
	...
	cl_mem     p_x_buff[BODIES], p_y_buff[BODIES], p_z_buff[BODIES], c_buff[BODIES], EPS_buff, tiling_factor_buff, outer_tile_start_buff[BODIES], outer_tile_end_buff[BODIES];
	cl_mem 	   a_x_buff[BODIES], a_y_buff[BODIES], a_z_buff[BODIES];

	unsigned int starts[BODIES], ends[BODIES];

	unsigned int tiling_factor = N_loc / TILE_ELEM;
	unsigned int a_size = N_loc / BODIES;
	unsigned int tile_to_kernel = tiling_factor / BODIES;
...
//	int new_N_loc = TILE_ELEM * tiling_factor;
    ...
    for(int i = 0; i < BODIES; i++){
    	starts[i] = i*tile_to_kernel;
    	ends[i] = (i+1)*tile_to_kernel;
    }
    ...
    cl_mem_ext_ptr_t p_x_ext[BODIES];
    cl_mem_ext_ptr_t p_y_ext[BODIES];
    cl_mem_ext_ptr_t p_z_ext[BODIES];
    cl_mem_ext_ptr_t c_ext[BODIES];
    cl_mem_ext_ptr_t a_x_ext[BODIES];
    cl_mem_ext_ptr_t a_y_ext[BODIES];
    cl_mem_ext_ptr_t a_z_ext[BODIES];
    cl_mem_ext_ptr_t EPS_ext;
    cl_mem_ext_ptr_t tiling_factor_ext;
    cl_mem_ext_ptr_t outer_tile_start_ext[BODIES];
    cl_mem_ext_ptr_t outer_tile_end_ext[BODIES];

    for(int i = 0; i < BODIES; i++){

    	p_x_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
    	p_x_ext[i].obj = 0;
    	p_x_ext[i].param = 0;

    	p_y_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
    	p_y_ext[i].obj = 0;
		p_y_ext[i].param = 0;

		p_z_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
		p_z_ext[i].obj = 0;
		p_z_ext[i].param = 0;

		a_x_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
		a_x_ext[i].obj = 0;
		a_x_ext[i].param = 0;

		a_y_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
		a_y_ext[i].obj = 0;
		a_y_ext[i].param = 0;

		a_z_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
		a_z_ext[i].obj = 0;
		a_z_ext[i].param = 0;

		c_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
		c_ext[i].obj = 0;
		c_ext[i].param = 0;

		outer_tile_start_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
		outer_tile_start_ext[i].obj = 0;
		outer_tile_start_ext[i].param = 0;

		outer_tile_end_ext[i].flags = (XCL_MEM_DDR_BANK0 << i);
		outer_tile_end_ext[i].obj = 0;
		outer_tile_end_ext[i].param = 0;
    }

    EPS_ext.flags = (XCL_MEM_DDR_BANK0);
	EPS_ext.obj = 0;
	EPS_ext.param = 0;

	tiling_factor_ext.flags = (XCL_MEM_DDR_BANK0);
	tiling_factor_ext.obj = 0;
	tiling_factor_ext.param = 0;

    for(int i = 0; i < BODIES; i++){

		p_x_buff[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(my_type) * N_loc, &(p_x_ext[i]), NULL);
		p_y_buff[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(my_type) * N_loc, &(p_y_ext[i]), NULL);
		p_z_buff[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(my_type) * N_loc, &(p_z_ext[i]), NULL);
		a_x_buff[i] = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(my_type) * a_size, &(a_x_ext[i]), NULL);
		a_y_buff[i] = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(my_type) * a_size, &(a_y_ext[i]), NULL);
		a_z_buff[i] = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(my_type) * a_size, &(a_z_ext[i]), NULL);
		c_buff[i] 	 = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(my_type) * N_loc, &(c_ext[i]), NULL);
		outer_tile_start_buff[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(unsigned int), &(outer_tile_start_ext[i]), NULL);
		outer_tile_end_buff[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(unsigned int), &(outer_tile_end_ext[i]), NULL);

    }

    EPS_buff = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(float), &EPS_ext, NULL);
    tiling_factor_buff = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, sizeof(unsigned int), &tiling_factor_ext, NULL);
    ...
    for (int t = 0; t < nt; t++) {
		...
		for(int i = 0; i < BODIES; i++){
			...
			error = clEnqueueWriteBuffer(commands, p_x_buff[i], CL_TRUE, 0, sizeof(my_type) * N_loc, p_x, 0, NULL, NULL);
			if (error != CL_SUCCESS) {
				printf("error while writing the buffer!! p_x\n");
				exit(EXIT_FAILURE);
			}
			error = clEnqueueWriteBuffer(commands, p_y_buff[i], CL_TRUE, 0, sizeof(my_type) * N_loc, p_y, 0, NULL, NULL);
			if (error != CL_SUCCESS) {
				printf("error while writing the buffer!! p_y\n");
				exit(EXIT_FAILURE);
			}
			error = clEnqueueWriteBuffer(commands, p_z_buff[i], CL_TRUE, 0, sizeof(my_type) * N_loc, p_z, 0, NULL, NULL);
			if (error != CL_SUCCESS) {
				printf("error while writing the buffer!! p_z\n");
				exit(EXIT_FAILURE);
			}
			error = clEnqueueWriteBuffer(commands, c_buff[i], CL_TRUE, 0, sizeof(my_type) * N_loc, m_local, 0, NULL, NULL);
			if (error != CL_SUCCESS) {
				printf("error while writing the buffer!! m\n");
				exit(EXIT_FAILURE);
			}
			error = clEnqueueWriteBuffer(commands, outer_tile_start_buff[i], CL_TRUE, 0, sizeof(unsigned int), &(starts[i]), 0, NULL, NULL);
			if (error != CL_SUCCESS) {
				printf("error while writing the buffer!! tiling_factor\n");
				exit(EXIT_FAILURE);
			}
			error = clEnqueueWriteBuffer(commands, outer_tile_end_buff[i], CL_TRUE, 0, sizeof(unsigned int), &(ends[i]), 0, NULL, NULL);
			if (error != CL_SUCCESS) {
				printf("error while writing the buffer!! tiling_factor\n");
				exit(EXIT_FAILURE);
			}
		...
		for(int i = 0; i < BODIES; i++, args+=9){

			error = clSetKernelArg(kernel, 0 + args, sizeof(cl_mem), &p_x_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
			}
			error = clSetKernelArg(kernel, 1 + args, sizeof(cl_mem), &p_y_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
			}
			error = clSetKernelArg(kernel, 2 + args, sizeof(cl_mem), &p_z_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
			}
			error = clSetKernelArg(kernel, 3 + args, sizeof(cl_mem), &a_x_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
			}
			error = clSetKernelArg(kernel, 4 + args, sizeof(cl_mem), &a_y_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
			}
			error = clSetKernelArg(kernel, 5 + args, sizeof(cl_mem), &a_z_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
			}
			error = clSetKernelArg(kernel, 6 + args, sizeof(cl_mem), &c_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
			}
			error = clSetKernelArg(kernel, 7 + args, sizeof(cl_mem), &outer_tile_start_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
			}
			error = clSetKernelArg(kernel, 8 + args, sizeof(cl_mem), &outer_tile_end_buff[i]);
			if (error != CL_SUCCESS) {
				printf("Error: Failed to set kernel arguments 0! %d\n", error);
				printf("Test failed\n");
				exit(EXIT_FAILURE);
		...
		error = clSetKernelArg(kernel, args++, sizeof(cl_mem), &EPS_buff);
		if (error != CL_SUCCESS) {
			printf("Error: Failed to set kernel arguments 0! %d\n", error);
			printf("Test failed\n");
			exit(EXIT_FAILURE);
		}
		error = clSetKernelArg(kernel, args++, sizeof(cl_mem), &tiling_factor_buff);
		if (error != CL_SUCCESS) {
			printf("Error: Failed to set kernel arguments 0! %d\n", error);
			printf("Test failed\n");
			exit(EXIT_FAILURE);
		...
		for(int i = 0; i < BODIES; i++){
			...
			error = clEnqueueReadBuffer(commands,  a_x_buff[i], CL_TRUE, 0, sizeof(my_type) * a_size, a_x + a_size * i, 0, NULL, &readEvent_0);
			error |= clEnqueueReadBuffer(commands, a_y_buff[i], CL_TRUE, 0, sizeof(my_type) * a_size, a_y + a_size * i, 0, NULL, &readEvent_1);
			error |= clEnqueueReadBuffer(commands, a_z_buff[i], CL_TRUE, 0, sizeof(my_type) * a_size, a_z + a_size * i, 0, NULL, &readEvent_2);
...
fpgarropencl_main(int argc, char **argv)
{
  ...
  #define TARGET_DEVICE GET_STRING(SDX_PLATFORM)
	...
	// Create Program Objects
	  ...
	  fake_program_spec_t fake_program = FakeProgramSpecBuilder::build(&nbody);
	  print_fake_program_spec(&fake_program);
	  unsigned char *kernelbinary = (unsigned char *)&fake_program;
	  size_t kernel_len = sizeof(fake_program);
	  // Create the compute program from offline
	  printf("create program with binary \n");
	  program = clCreateProgramWithBinary(
	      context, 1, &device_id, &kernel_len,
	      (const unsigned char **)&kernelbinary, &status, &err);
	  if ((!program) || (err != CL_SUCCESS)) {
	    printf(
		"Error: Failed to create compute program from binary "
		"%d!\n",
		err);
	    printf("Test failed\n");
	    return EXIT_FAILURE;
