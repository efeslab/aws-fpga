...
// ==============================================================
`timescale 1ns/1ps
module face_detect_control_s_axi
#(parameter
    ...
    output wire [63:0]                   result_w,
    output wire [63:0]                   result_h
);
//------------------------Address Info-------------------
...
// 0x20 : reserved
// 0x24 : Data signal of result_x
//        bit 31~0 - result_x[31:0] (Read/Write)
// 0x28 : Data signal of result_x
//        bit 31~0 - result_x[63:32] (Read/Write)
// 0x2c : reserved
// 0x30 : Data signal of result_y
//        bit 31~0 - result_y[31:0] (Read/Write)
// 0x34 : Data signal of result_y
//        bit 31~0 - result_y[63:32] (Read/Write)
// 0x38 : reserved
// 0x3c : Data signal of result_w
//        bit 31~0 - result_w[31:0] (Read/Write)
// 0x40 : Data signal of result_w
//        bit 31~0 - result_w[63:32] (Read/Write)
// 0x44 : reserved
// 0x48 : Data signal of result_h
//        bit 31~0 - result_h[31:0] (Read/Write)
// 0x4c : Data signal of result_h
//        bit 31~0 - result_h[63:32] (Read/Write)
// 0x50 : reserved
...
localparam
    ...
    ADDR_INDATA_DATA_0   = 7'h18,
    ADDR_INDATA_DATA_1   = 7'h1c,
    ADDR_INDATA_CTRL     = 7'h20,
    ADDR_RESULT_X_DATA_0 = 7'h24,
    ADDR_RESULT_X_DATA_1 = 7'h28,
    ADDR_RESULT_X_CTRL   = 7'h2c,
    ADDR_RESULT_Y_DATA_0 = 7'h30,
    ADDR_RESULT_Y_DATA_1 = 7'h34,
    ADDR_RESULT_Y_CTRL   = 7'h38,
    ADDR_RESULT_W_DATA_0 = 7'h3c,
    ADDR_RESULT_W_DATA_1 = 7'h40,
    ADDR_RESULT_W_CTRL   = 7'h44,
    ADDR_RESULT_H_DATA_0 = 7'h48,
    ADDR_RESULT_H_DATA_1 = 7'h4c,
    ADDR_RESULT_H_CTRL   = 7'h50,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_result_w = 'b0;
    reg  [63:0]                   int_result_h = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_RESULT_W_DATA_0: begin
                    rdata <= int_result_w[31:0];
                end
                ADDR_RESULT_W_DATA_1: begin
                    rdata <= int_result_w[63:32];
                end
                ADDR_RESULT_H_DATA_0: begin
                    rdata <= int_result_h[31:0];
                end
                ADDR_RESULT_H_DATA_1: begin
                    rdata <= int_result_h[63:32];
                end
            endcase
        end
    end
...
assign result_w    = int_result_w;
assign result_h    = int_result_h;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RESULT_W_DATA_0)
            int_result_w[31:0] <= (WDATA[31:0] & wmask) | (int_result_w[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_result_w[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_RESULT_W_DATA_1)
            int_result_w[63:32] <= (WDATA[31:0] & wmask) | (int_result_w[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RESULT_H_DATA_0)
            int_result_h[31:0] <= (WDATA[31:0] & wmask) | (int_result_h[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_result_h[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_RESULT_H_DATA_1)
            int_result_h[63:32] <= (WDATA[31:0] & wmask) | (int_result_h[63:32] & ~wmask);
    end
end
