...
entity calc_0_control_s_axi is
generic (
...
end entity calc_0_control_s_axi;
...
-- 0x30 : reserved
-- 0x34 : Data signal of scalar
--        bit 31~0 - scalar[31:0] (Read/Write)
-- 0x38 : reserved
-- 0x3c : Data signal of array_size
--        bit 31~0 - array_size[31:0] (Read/Write)
-- 0x40 : reserved
-- 0x44 : Data signal of operation_type
--        bit 31~0 - operation_type[31:0] (Read/Write)
-- 0x48 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of calc_0_control_s_axi is
    ...
    constant ADDR_SCALAR_DATA_0         : INTEGER := 16#34#;
    constant ADDR_SCALAR_CTRL           : INTEGER := 16#38#;
    constant ADDR_ARRAY_SIZE_DATA_0     : INTEGER := 16#3c#;
    constant ADDR_ARRAY_SIZE_CTRL       : INTEGER := 16#40#;
    constant ADDR_OPERATION_TYPE_DATA_0 : INTEGER := 16#44#;
    constant ADDR_OPERATION_TYPE_CTRL   : INTEGER := 16#48#;
    ...
    signal int_scalar          : UNSIGNED(31 downto 0) := (others => '0');
    signal int_array_size      : UNSIGNED(31 downto 0) := (others => '0');
    signal int_operation_type  : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SCALAR_DATA_0 =>
                        rdata_data <= RESIZE(int_scalar(31 downto 0), 32);
                    when ADDR_ARRAY_SIZE_DATA_0 =>
                        rdata_data <= RESIZE(int_array_size(31 downto 0), 32);
                    when ADDR_OPERATION_TYPE_DATA_0 =>
                        rdata_data <= RESIZE(int_operation_type(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    scalar               <= STD_LOGIC_VECTOR(int_scalar);
    array_size           <= STD_LOGIC_VECTOR(int_array_size);
    operation_type       <= STD_LOGIC_VECTOR(int_operation_type);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SCALAR_DATA_0) then
                    int_scalar(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_scalar(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_OPERATION_TYPE_DATA_0) then
                    int_operation_type(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_operation_type(31 downto 0));
                end if;
            end if;
        end if;
    end process;
