...
localparam
    ...
    ADDR_SCALAR_DATA_0         = 7'h34,
    ADDR_SCALAR_CTRL           = 7'h38,
    ADDR_ARRAY_SIZE_DATA_0     = 7'h3c,
    ADDR_ARRAY_SIZE_CTRL       = 7'h40,
    ADDR_OPERATION_TYPE_DATA_0 = 7'h44,
    ADDR_OPERATION_TYPE_CTRL   = 7'h48,
    WRIDLE                     = 2'd0,
    WRDATA                     = 2'd1,
    WRRESP                     = 2'd2,
    WRRESET                    = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_scalar = 'b0;
    reg  [31:0]                   int_array_size = 'b0;
    reg  [31:0]                   int_operation_type = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_SCALAR_DATA_0: begin
                    rdata <= int_scalar[31:0];
                end
                ADDR_ARRAY_SIZE_DATA_0: begin
                    rdata <= int_array_size[31:0];
                end
                ADDR_OPERATION_TYPE_DATA_0: begin
                    rdata <= int_operation_type[31:0];
                end
            endcase
        end
    end
...
assign scalar         = int_scalar;
assign array_size     = int_array_size;
assign operation_type = int_operation_type;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_SCALAR_DATA_0)
            int_scalar[31:0] <= (WDATA[31:0] & wmask) | (int_scalar[31:0] & ~wmask);
    end
end
...
// int_operation_type[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_operation_type[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OPERATION_TYPE_DATA_0)
            int_operation_type[31:0] <= (WDATA[31:0] & wmask) | (int_operation_type[31:0] & ~wmask);
    end
end
