...
for intfname in INTFNAMES:
    intf_toggles.add_argument(f"--{intfname}", dest=intfname,
                              action="store_true", default=True, help=f"enable {intfname} (default: %(default)s)")
...
class AXIIntf(object):
    ...
    def getRecordBusName(self) -> str:
        return f"rr_{self.intfname}_SH2CL_logging_bus"

    def getValidateBusName(self) -> str:
        return f"rr_{self.intfname}_CL2SH_logging_bus"

    def getReplayBusName(self) -> str:
        return f"rr_{self.intfname}_replay_bus"
...
def exportMergeStructure(filepath: Path,
    ...
    with open(filepath, "w") as f:
        ...
        for intf in ignoredIntfs:
            print(ignore_callback(intf), file=f)
...
with open(output_dir/args.cfg_out, "w") as f:
    ...
    placement_vec_entries = [
        "{} /*{}*/".format(INTFCFG[intf.intfname]["PLACEMENT"], intf.intfname)
        for intf in RRIntfs
    ]
    print(
