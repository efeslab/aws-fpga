...
int fpga_hugealloc_get_all_pages(struct fpga_hugepage_desc **descs) {
    ...
    fail_on(ptr == NULL, err_free, "Error mapping huge pages");
    ...
    *descs = malloc(npages * sizeof(struct fpga_hugepage_desc));
    ...
    for (uint64_t i = 0; i < npages; i++) {
        ...
        fail_on((uint64_t)actual_off != vfn * 8, err_free, "lseek failed");

        ret = read(fd, &pe, sizeof(pe));
        fail_on(ret != sizeof(pe), err_free, "Error reading pagemap");
